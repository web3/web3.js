...
var Contract = function(jsonInterface, address, options) {
    ...
    this.off = this._removeListeners.bind({type: 'removeListener', parent: this});
...
Contract.prototype._newListener = function(event, options, func){
    var _this = this.parent,
        args = Array.prototype.slice.call(arguments),
        event = _this.jsonInterface.find(function(json){
    ...
    _this._checkListener('newListener', event.name, func);
    _this._checkListener('removeListener', event.name, func);


    var subscriptionParams = _this._encodeEventABI(event ,options);
        subscriptionId = sha3(event.name + subscriptionParams.topics.join('')).substr(0,8);
    // TODO always create new subscription?? or check if options are the same
    ...
    if(!_this._subscriptions[subscriptionId]) {
        _this._subscriptions[subscriptionId] = new Subscription({
            subscription: {
                params: 1,
        ...
        //_this._subscriptions[subScriptionId].subscribe('log', {
    ...
    if(func) {
        //_this['_'+ this.type](event.name, func);
        _this._subscriptions[subscriptionId].on('data', func);
    }

    console.log('ADDED', event.name +'('+ subscriptionId +')' , func);

    return _this._subscriptions[subscriptionId];
};
...
Contract.prototype._removeListeners = function(event, func){
    ...
    if(_this._subscriptions[event]) {
        ...
        if(!_this.listenerCount(event) || this.type === 'removeAllListeners') {
            ...
            _this._subscriptions[event].removeListener('data', func);
        }
