...
Contract.prototype._newListener = function(event, options, func){
    ...
    console.log(_this._encodeEventABI(event ,options));

    var formatters = {};

    // create new subscription
    if(!_this._subscriptions[event.name]) {
        _this._subscriptions[event.name] = new Subscription({
            subscription: {
                params: 1,
    ...
    if(func) {
        ...
        _this._subscriptions[event.name].on('data', func);
    }

    console.log('ADDED', event.name, func);

    return _this._subscriptions[event.name];
};
