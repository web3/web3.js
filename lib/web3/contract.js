 ...
 */
Contract.prototype._encodeParams = function (method, params) {
    return this.jsonInterface.filter(function (json) {
        return ((method === 'constructor' && json.type === method) || (json.name === method && json.type === 'function')) && json.inputs.length === params.length;
    }).map(function (json) {
...
Contract.prototype.deploy = function(options, callback){
    ...
    options.data = options.data || _this.options.data;
    options.from = options.from || _this.options.from;
    options.gasPrice = options.gasPrice || _this.options.gasPrice;
    options.gasLimit = options.gasLimit || _this.options.gasLimit;

    // return error, if no "from" is specified
    if(!utils.isAddress(options.from)) {
        return _this._fireError(new Error('No "from" address specified in either the default options, or the given options.'), defer.promise, defer.reject, callback);
    }

    // return error, if no "data" is specified
    if(!options.data) {
        return _this._fireError(new Error('No "data" specified in either the default options, or the given options.'), defer.promise, defer.reject, callback);
    }

    // add constructor parameters
    var bytes = _this._encodeParams('constructor', options.arguments);
    options.data += bytes;


    // send the actual deploy transaction
    _this._web3.eth.sendTransaction({
        from: options.from,
...
Contract.prototype.encodeABI = function(options){
    ...
    // add the function signature
        ...
        bytes += sha3(options.method).slice(0, 8);

    // add the parameters
    bytes += this._encodeParams(options.method, options.arguments);
