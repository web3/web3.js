    ...
    describe("method.encodeABI", function() {
        it("should handle bytes32 arrays that only contain 1 byte", function() {
            ...
            var result = contract.methods
                .takesTwoBytes32(
                    "0x".concat("a".repeat(2)),
                    "0x".concat("b".repeat(2))
                )
                .encodeABI();
        ...
        it("should throw an exception on bytes32 arrays that have an invalid length", function() {
            ...
            assert.throws(
                test,
                'Given parameter bytes has an invalid length: "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"'
            );
        });
        ...
        it("should throw an exception on bytes32 arrays that are too long", function() {
            ...
            assert.throws(
                test,
                'Given parameter bytes is too long: "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"'
            );
        });
    });
});
