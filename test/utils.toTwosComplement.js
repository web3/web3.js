...
var tests = [
    {
        value: 1,
        expected: utils.padLeft(new BigNumber(1).round().toString(16), 64)
    },
    { value: "1", expected: utils.padLeft(new BigNumber(1).toString(16), 64) },
    {
        value: "0x1",
        expected: utils.padLeft(new BigNumber(1).toString(16), 64)
    },
    {
        value: "15",
        expected: utils.padLeft(new BigNumber(15).toString(16), 64)
    },
    {
        value: "0xf",
        expected: utils.padLeft(new BigNumber(15).toString(16), 64)
    ...
    {
        value: "0x0",
        expected: utils.padLeft(new BigNumber(0).toString(16), 64)
    },
    { value: -0, expected: utils.padLeft(new BigNumber(0).toString(16), 64) },
    { value: "-0", expected: utils.padLeft(new BigNumber(0).toString(16), 64) },
    {
    ...
    describe("toTwosComplement", function() {
        tests.forEach(function(test) {
            it("printing " + test.value, function() {
                assert.equal(
                    utils.toTwosComplement(test.value).replace("0x", ""),
                    test.expected
                );
            });
        });
