        ...
        tests.forEach(function(test, i) {
            if (test.error) {
                it("signTransaction must error", function(done) {
                    ...
                    var testAccount = ethAccounts.privateKeyToAccount(
                        test.privateKey
                    );
                    assert.equal(testAccount.address, test.address);

                    testAccount
                        .signTransaction(test.transaction)
                        .catch(function(err) {
                ...
                it("recoverTransaction, must recover signature", function() {
                    ...
                    testAccount
                        ...
                        .then(function(tx) {
                            assert.equal(
                                ethAccounts.recoverTransaction(
                                    tx.rawTransaction
                                ),
                                test.address
                            );
                ...
                it("recoverTransaction, must also recover old signature from eth-signer", function() {
                    ...
                    assert.equal(
                        ethAccounts.recoverTransaction(oldSignature),
                        test.address
                    );
                });
            }
