...
var coder = require('ethjs-abi');

// TODO check line 108 again!

var convertResult = function (result) {
    var resultArray = [];
    _.each(result, function (res, i) {
        if(isFinite(i))
            resultArray.push(res);
    });
    var convert = function (resultArray) {
        return _.map(resultArray, function (res) {
            if(res.constructor.name === 'BN') {
                res = res.toString(10);
            } else if(_.isArray(res)) {
                res = convert(res);
            } else if(_.isString(res) && utils.isAddress(res)) {
                res = utils.toChecksumAddress(res);
            }
            return res;
        });
    };
    ...
    describe('decodeParam', function () {
        var test = function (t) {
            it('should turn ' + t.type +': ' + t.value + ' to ' + t.expected, function () {
                ...
                var value = (typeof t.value === 'string') ? '0x'+ t.value.replace('0x','') : t.value;
                var result = coder.decodeMethod({type: 'function', outputs: [{type: t.type, name: 'TestName'}]}, value);


                var resultArray = convertResult(result);
    ...
    describe('decodeParams', function () {
        var test = function (t) {
            it('should turn ' + t.values + ' to ' + t.expected, function () {
                ...
                var outputs = t.types.map(function (type, i) {
                    return {type: type, name: 'TestName'+ i};
                });

                var value = (typeof t.values === 'string') ? '0x'+ t.values.replace('0x','') : t.values;
                var result = coder.decodeMethod({type: 'function', outputs: outputs}, value);
