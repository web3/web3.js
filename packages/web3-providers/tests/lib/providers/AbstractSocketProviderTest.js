...
describe('AbstractSocketProviderTest', () => {
    ...
    it('constructor check', () => {
        ...
        expect(abstractSocketProvider.timeout)
            .toEqual(0);

        expect(abstractSocketProvider.subscriptions)
            .toEqual({});
        ...
        expect(abstractSocketProvider.send)
            .toBeInstanceOf(Function);

        expect(abstractSocketProvider.sendBatch)
            .toBeInstanceOf(Function);

        expect(abstractSocketProvider.registerEventListeners)
            .toBeInstanceOf(Function);

        expect(abstractSocketProvider)
    ...
    it('calls removeAllListeners with a specific event', () => {
        ...
        expect(abstractSocketProvider.listenerCount('testing'))
            .toEqual(0);
    });
    ...
    it('calls onConnect and connect event will be emitted', async () => {
        ...
        expect(abstractSocketProvider.subscriptions['0x0'].id)
            .toEqual('0x1');
    ...
    it('calls subscribe and returns a resolved promise with a subscription ID', async () => {
        ...
        expect(abstractSocketProvider.subscriptions['0x1'].id)
            .toEqual('0x1');
    ...
    it('calls subscribe and returns a resolved promise with a subscription ID', async done => {
        abstractSocketProvider.send = jest.fn((subscribeMethod, parameters) => {
            expect(subscribeMethod)
                .toEqual('shh_subscribe');
        ...
        await expect(abstractSocketProvider.subscribe('shh_subscribe', 'messages', [])).rejects
            .toThrow('NOPE')
            .then(() => {
                expect(abstractSocketProvider.send)
                    .toHaveBeenCalled();
    ...
    it('calls unsubscribe and subscription id does not exist', () => {
        expect(abstractSocketProvider.unsubscribe('no',  'eth_unsubscribe')).rejects
            .toThrow('Provider error: Subscription with ID no does not exist.');
    });

    it('calls unsubscribe and subscription id does not exist', async () => {
        ...
        expect(abstractSocketProvider.subscriptions['0x0'])
            .toBeUndefined();
    });
    ...
    it('calls clearSubscriptions and one unsubscribe call returns false', async () => {
        ...
        await expect(abstractSocketProvider.clearSubscriptions('eth_unsubscribe')).rejects
            .toThrow(`Could not unsubscribe all subscriptions: ${JSON.stringify([false])}`);
    ...
    it('calls getSubscriptionEvent and has to iterate over all items', () => {
       ...
       expect(abstractSocketProvider.getSubscriptionEvent('0x0'))
           .toEqual('ID');
    });
});
