	...
	describe('Contract functions and defaults', () => {
		it('should deploy contract', async () => {
			...
			const deploySpy = jest
				...
				.mockImplementation((_objInstance, tx, _returnformat) => {
					expect(tx.to).toBeUndefined();
					expect(tx.gas).toStrictEqual(gas);
					expect(tx.gasPrice).toBeUndefined();
					expect(tx.from).toStrictEqual(from);
		...
		// eslint-disable-next-line @typescript-eslint/require-await
		it('send on deployed contract should work', async () => {
			const from = '0x12364916b10Ae90076dDa6dE756EE1395BB69ec2';
			const gas = '1000000';
			const sendOptions = { from, gas };
			const arg = 'Hello';
		...
		it('call on deployed contract should decode result', async () => {
			...
			const spyEthCall = jest.spyOn(eth, 'call').mockImplementation((_objInstance, _tx) => {
				...
				// eslint-disable-next-line @typescript-eslint/no-unsafe-return
				return Promise.resolve(encodedArg) as any; // contract class should decode it
			});
		...
		it('clone pre deployed contract with address', () => {
			const contract = new Contract(
		...
		it('defaults set and get', () => {
			...
			// expect(contract.transactionPollingInterval).toStrictEqual(5000);
			...
			expect(contract.transactionReceiptPollingInterval).toBeUndefined();
			...
			expect(contract.transactionConfirmationPollingInterval).toBeUndefined();
