...
describe('ContractDeployMethodTest', () => {
    ...
    it('constructor check', () => {
        expect(contractDeployMethod.utils)
            .toEqual(Utils);

        expect(contractDeployMethod.formatters)
            .toEqual(formatters);

        expect(contractDeployMethod.transactionConfirmationWorkflow)
            .toEqual(transactionConfirmationWorkflowMock);

        expect(contractDeployMethod.accounts)
            .toEqual(accountsMock);

        expect(contractDeployMethod.transactionSigner)
    ...
    it('calls beforeExecution and removes the to property from the options', () => {
        ...
        expect(contractDeployMethod.parameters[0].to)
            .toBeUndefined();
    ...
    it('calls afterExecution and returns the cloned contract object', () => {
        contractMock.clone
            .mockReturnValueOnce({options: {address: ''}});

        const clonedContract = contractDeployMethod.afterExecution({contractAddress: '0x0'});

        expect(clonedContract.options.address)
            .toEqual('0x0');

        expect(contractMock.clone)
            .toHaveBeenCalled();
    });
});
