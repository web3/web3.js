        ...
        event.toLowerCase() === "allevents"
            ...
            : this.options.jsonInterface.find(function(json) {
                  return (
                      json.type === "event" &&
                      (json.name === event ||
                          json.signature === "0x" + event.replace("0x", ""))
                  );
              });

    if (!event) {
        throw new Error(
            'Event "' + event.name + "\" doesn't exist in this contract."
        );
    }
        ...
        switch (args.type) {
            ...
            case "send":
                ...
                var sendTransaction = new Method({
                    ...
                    requestManager: _this._parent._requestManager,
                    accounts:
                        _this.constructor._ethAccounts || _this._ethAccounts, // is eth.accounts (necessary for wallet signing)
                    defaultAccount: _this._parent.defaultAccount,
                    defaultBlock: _this._parent.defaultBlock,
