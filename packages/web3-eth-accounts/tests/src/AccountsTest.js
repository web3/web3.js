...
describe('AccountsTest', () => {
    ...
    beforeEach(() => {
        ...
        accounts = new Accounts(providerMock, providersModuleFactoryMock, methodModuleFactoryMock, methodFactory, Utils, formatters, {});
    });

    it('constructor check', () => {
        expect(accounts.utils)
            .toEqual(Utils);

        expect(accounts.formatters)
            .toEqual(formatters);

        expect(accounts.wallet.defaultKeyName)
            .toEqual('web3js_wallet');
    ...
    it('JSON-RPC methods check', () => {
        expect(accounts.methodFactory.methods)
            .toEqual({
    ...
    it('calls addAccountFunctions and returns the expected object', () => {
        ...
        expect(accounts._addAccountFunctions(object))
            .toBeInstanceOf(Object);

        expect(object.signTransaction)
    ...
    it('calls create with the entropy parameter and returns the expected object', () => {
        ...
        expect(accounts.create('entropy'))
            .toBeInstanceOf(Object);
    ...
    it('calls privateKeyToAccount with the privateKey parameter and returns the expected object', () => {
        ...
        expect(accounts.privateKeyToAccount('pk'))
            .toBeInstanceOf(Object);
    ...
    it('calls signTransaction and returns a resolved promise', async () => {
        ...
        Account.decodeSignature
            .mockReturnValueOnce(['seven', 'eight', 'nine']);

        RLP.decode
            .mockReturnValueOnce(['zero', 'one', 'two', 'three', 'four', 'five', 'six'])
            .mockReturnValueOnce(['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight']);

        await expect(accounts.signTransaction(tx, 'pk', callback)).resolves
            .toEqual({
                messageHash: 'hash',
                v: 'six',
                r: 'seven',
        ...
        expect(Bytes.fromNat)
            .toHaveBeenNthCalledWith(1, 2);

        expect(Bytes.fromNat)
            .toHaveBeenNthCalledWith(2, 3);

        expect(Bytes.fromNat)
            .toHaveBeenNthCalledWith(3, 1);

        expect(Bytes.fromNat)
            .toHaveBeenNthCalledWith(4, 5);

        expect(Bytes.fromNat)
            .toHaveBeenNthCalledWith(5, 1);

        expect(RLP.encode)
            .toHaveBeenNthCalledWith(1, [1, 1, 1, 'lowercase', 1, 'data', 1, '0x', '0x']);

        expect(RLP.encode)
            .toHaveBeenNthCalledWith(2, ['zero', 'one', 'two', 'three', 'four', 'five', 'seven', 'eight', 'nine']);

        expect(Hash.keccak256)
            .toHaveBeenNthCalledWith(1, 'encoded');

        expect(Hash.keccak256)
            .toHaveBeenNthCalledWith(2, 'encoded');

        expect(Nat.toNumber)
            .toHaveBeenCalledWith(1);

        expect(Account.makeSigner)
            .toHaveBeenCalledWith(37);

        expect(signer)
            .toHaveBeenCalledWith('hash', 'pk');

        expect(RLP.decode)
            .toHaveBeenCalledWith('encoded');

        expect(Account.decodeSignature)
            .toHaveBeenCalledWith('signature');
    });
    ...
    it('calls signTransaction without chainId, gasPrice, nonce and returns a resolved promise', async () => {
        ...
        accounts.getGasPrice
            .mockReturnValueOnce(Promise.resolve(3));

        accounts.getTransactionCount
        ...
        expect(accounts.getTransactionCount)
            .toHaveBeenCalledWith('0x0');

        expect(accounts.getId)
            .toHaveBeenCalled();

        expect(accounts.getGasPrice)
            .toHaveBeenCalled();
    });
    ...
    it('calls singTransaction and returns a rejected promise because it could not fetch the missing properties', async () => {
        ...
        await expect(accounts.signTransaction({}, 'pk', () => {
        })).rejects
            .toThrow(`One of the values 'chainId', 'gasPrice', or 'nonce' couldn't be fetched: ${JSON.stringify([null, null, null])}`);
    ...
    it('calls singTransaction and returns a rejected promise because of invalid values in the TX', async () => {
        ...
        await expect(accounts.signTransaction(tx, 'pk', () => {
        })).rejects
            .toThrow('Gas, gasPrice, nonce or chainId is lower than 0');
    });
    ...
    it('calls singTransaction and returns a rejected promise because the gas limit property is missing', async () => {
        ...
        await expect(accounts.signTransaction(tx, 'pk', callback)).rejects
            .toThrow('ERROR');
    ...
    it('calls singTransaction and returns a rejected promise because of the missing TX parameter', async () => {
        ...
        await expect(accounts.signTransaction(undefined, 'pk', callback)).rejects
            .toThrow('No transaction object given!');

        expect(callback)
            .toHaveBeenCalledWith(new Error('No transaction object given!'));
    });

    it('calls recoverTransaction and returns the expected string', () => {
        RLP.decode = jest.fn(rawTransaction => {
            expect(rawTransaction)
                .toEqual('rawTransaction');
        ...
        RLP.encode = jest.fn(signingData => {
            expect(signingData)
                .toEqual([0, 1, 2, 3, 4, 5, 1, '0x', '0x']);

            return 'encoded';
        });

        Hash.keccak256 = jest.fn(signingDataHex => {
            expect(signingDataHex)
                .toEqual('encoded');
        ...
        expect(accounts.recoverTransaction('rawTransaction'))
            .toEqual('recovered');

        expect(Account.recover)
            .toHaveBeenCalled();
        ...
        expect(Bytes.fromNumber)
            .toHaveBeenCalled();

        expect(Bytes.toNumber)
            .toHaveBeenCalled();

        expect(Account.encodeSignature)
            .toHaveBeenCalled();
    ...
    it('calls hashMessage with strict hex and returns the expected string', () => {
        ...
        expect(accounts.hashMessage('data'))
            .toEqual('keccak');

        expect(Utils.isHexStrict)
            .toHaveBeenCalled();

        expect(Utils.hexToBytes)
            .toHaveBeenCalled();
    ...
    it('calls hashMessage with non-strict hex and returns the expected string', () => {
        ...
        expect(accounts.hashMessage('message'))
            .toEqual('keccak');
    ...
    it('calls sign with non-strict hex and returns the expected string', () => {
        ...
        expect(accounts.sign('message', 'pk'))
            .toEqual({
                message: 'message',
                messageHash: 'keccak',
                v: 'v',
    ...
    it('calls recover with a string as message and returns the expected value', () => {
        ...
        expect(accounts.recover('message', 'signature', false))
            .toEqual('recovered');
    ...
    it('calls recover with a string as message, preFixed is true and it returns the expected value', () => {
        ...
        expect(accounts.recover('message', 'v', 'r', 's', true))
            .toEqual('recovered');
    ...
    it('calls decrypt and returns the expected object', () => {
        ...
        Account.fromPrivate = jest.fn(seed => {
            expect(seed)
                .toEqual(`0x${Buffer.concat([Buffer.from('0'), Buffer.from('0')]).toString('hex')}`);

            return object;
        });

        scryptsy
            .mockReturnValueOnce(Buffer.from('00000000000000000000000000000000'));

        Utils.sha3
            .mockReturnValueOnce('0xmac');
        ...
        crypto.createDecipheriv = jest.fn((cipher, derivedKey, buffer) => {
            ...
            expect(derivedKey)
                .toEqual(Buffer.from('0000000000000000'));

            expect(buffer)
                .toEqual(Buffer.from(['0x0'], 'hex'));

            return decipher;
        });

        expect(accounts.decrypt(json, 'password', false))
            .toEqual(object);

        expect(scryptsy)
            .toHaveBeenCalledWith(Buffer.from('password'), Buffer.from('salt', 'hex'), 'n', 'r', 'p', 'dklen');

        expect(Utils.sha3)
            .toHaveBeenCalledWith(Buffer.concat([Buffer.from('0000000000000000'), Buffer.from(json.crypto.ciphertext, 'hex')]));

        expect(crypto.createDecipheriv)
            .toHaveBeenCalled();

        expect(decipher.update)
            .toHaveBeenCalledWith(Buffer.from(json.crypto.ciphertext, 'hex'));

        expect(decipher.final)
            .toHaveBeenCalled();
    ...
    it('calls decrypt with pbkdf2 and returns the expected object', () => {
        ...
        crypto.pbkdf2Sync = jest.fn((password, salt, c, dklen, sha256) => {
            expect(password)
                .toEqual(Buffer.from(password));

            expect(salt)
                .toEqual(Buffer.from('salt', 'hex'));
    ...
    it('calls encrypt and returns the expected object', () => {
        ...
        crypto.randomBytes
            .mockReturnValue(Buffer.from('random'));
        ...
        scryptsy
            .mockReturnValueOnce(Buffer.from('0000000000000000'));
        ...
        expect(accounts.encrypt('pk', 'password', options))
            .toEqual({
                version: 3,
                id: 0,
                address: 'a',
        ...
        expect(crypto.randomBytes)
            .toHaveBeenNthCalledWith(1, 32);

        expect(crypto.randomBytes)
            .toHaveBeenNthCalledWith(2, 16);

        expect(crypto.randomBytes)
            .toHaveBeenNthCalledWith(3, 16);

        expect(scryptsy)
            .toHaveBeenCalledWith(Buffer.from('password'), Buffer.from('random'), 8192, 8, 1, 32);

        expect(crypto.createCipheriv)
            .toHaveBeenCalledWith('aes-128-ctr', Buffer.from('0000000000000000').slice(0, 16), Buffer.from('random'));

        expect(cipher.update)
            .toHaveBeenCalledWith(Buffer.from(account.privateKey.replace('0x', ''), 'hex'));
        ...
        expect(uuid.v4)
            .toHaveBeenCalledWith({random: Buffer.from('random')});
    });
    ...
    it('calls encrypt with the pbkdf2 sheme and returns the expected object', () => {
        ...
        expect(crypto.pbkdf2Sync)
            .toHaveBeenCalledWith(Buffer.from('password'), Buffer.from('random'), 262144, 32, 'sha256');
