...
describe('SyncingSubscriptionTest', () => {
    ...
    it('constructor check', () => {
        expect(syncingSubscription.isSyncing)
            .toEqual(null);

        expect(syncingSubscription.method)
            .toEqual('syncing');

        expect(syncingSubscription.type)
            .toEqual('eth_subscribe');

        expect(syncingSubscription.options)
            .toEqual(null);

        expect(syncingSubscription.utils)
            .toEqual({});

        expect(syncingSubscription.formatters)
            .toEqual(formatters);

        expect(syncingSubscription.moduleInstance)
            .toEqual({});
    });
