...
> extends Eip1193Provider {
	...
	public constructor(network: Network, transport: Transport, token: string, host: string) {
		super();
	...
	>(
		...
		if (this.transport === Transport.HTTPS) {
			const res = (await (this.provider as HttpProvider).request(
				payload,
				requestOptions,
			...
			if (
				...
				!isNullish(res.error) &&
				'code' in res.error &&
				(res.error as { code: number }).code === 429
			) {
