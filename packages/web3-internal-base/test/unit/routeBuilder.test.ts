...
beforeAll(() => {
    ...
    base = new Base(provider, testAPISchema, { protectProvider: true })
})
...
it('Should build expectedComputedRoute with 1 parameter', () => {
    ...
    const computedRoute = base.routeBuilder(testAPISchema.methods[0].route, routeParameter)
...
it('Should throw missing parameter error', () => {
    ...
    expect(() => {
        // @ts-ignore routeBuilder is private
        base.routeBuilder(testAPISchema.methods[0].route, routeParameter)
...
it('Should build expectedComputedRoute with 0 parameters', () => {
    ...
    // @ts-ignore routeBuilder is private
    const computedRoute = base.routeBuilder(testAPISchema.methods[1].route, routeParameter)
    expect(computedRoute).toBe(expectedComputedRoute)
})
...
it('Should build expectedComputedRoute with 4 parameters', () => {
    ...
    // @ts-ignore routeBuilder is private
    const computedRoute = base.routeBuilder(testAPISchema.methods[2].route, routeParameters)
    expect(computedRoute).toBe(expectedComputedRoute)
})