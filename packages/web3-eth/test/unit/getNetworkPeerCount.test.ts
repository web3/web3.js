...
describe('getNetworkPeerCount', () => {
    ...
    it('should make request with expected requestArguments', async () => {
        ...
        const result = await web3Eth.getNetworkPeerCount();
    ...
    describe('Output formatting', () => {
        it('should convert RPC result to number', async () => {
            ...
            const result = await web3Eth.getNetworkPeerCount({ returnType: ValidTypesEnum.Number });
            expect(result).toStrictEqual(expectedResult);
            expect(web3ProvidersHttpRequestSpy).toHaveBeenCalledWith({
                method: 'net_peerCount',
                params: []
        ...
        it('should convert RPC result to number string', async () => {
            ...
            const result = await web3Eth.getNetworkPeerCount({ returnType: ValidTypesEnum.NumberString });
            expect(result).toStrictEqual(expectedResult);
            expect(web3ProvidersHttpRequestSpy).toHaveBeenCalledWith({
                method: 'net_peerCount',
                params: []
        ...
        it('should convert RPC result to BigInt', async () => {
            ...
            const result = await web3Eth.getNetworkPeerCount({ returnType: ValidTypesEnum.BigInt });
            expect(result).toStrictEqual(expectedResult);
            expect(web3ProvidersHttpRequestSpy).toHaveBeenCalledWith({
                method: 'net_peerCount',
                params: []
