...
import { DEFAULT_RETURN_FORMAT, FMT_BYTES, FMT_NUMBER, EthExecutionAPI,Address,UserOperation } from 'web3-types';
import { ethRpcMethods } from 'web3-rpc-methods';
import { sendUserOperation } from '../../../src/rpc_method_wrappers';
...
describe('sendUserOperation', () => {
	...
	it('should call rpcMethods.sendUserOperation with expected parameters', async () => {
		await sendUserOperation(web3Context, userOperation, entryPoint, DEFAULT_RETURN_FORMAT);
		expect(ethRpcMethods.sendUserOperation).toHaveBeenCalledWith(web3Context.requestManager, userOperation, entryPoint);
	});

	it('should format mockRpcResponse using provided return format', async () => {
		const mockRpcResponse = '0xe554d0701f7fdc734f84927d109537f1ac4ee4ebfa3670c71d224a4fa15dbcd1';
		...
		const result = await sendUserOperation(web3Context, userOperation, entryPoint, expectedReturnFormat);
