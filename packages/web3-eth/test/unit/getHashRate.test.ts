...
describe('getHashRate', () => {
    ...
    it('should make request with expected requestArguments', async () => {
        ...
        const result = await web3Eth.getHashRate();
    ...
    describe('Output formatting', () => {
        it('should convert RPC result to number', async () => {
            ...
            const result = await web3Eth.getHashRate({ returnType: ValidTypesEnum.Number });
            expect(result).toStrictEqual(expectedResult);
            expect(web3ProvidersHttpRequestSpy).toHaveBeenCalledWith({
                method: 'eth_hashrate',
                params: []
        ...
        it('should convert RPC result to number string', async () => {
            ...
            const result = await web3Eth.getHashRate({ returnType: ValidTypesEnum.NumberString });
            expect(result).toStrictEqual(expectedResult);
            expect(web3ProvidersHttpRequestSpy).toHaveBeenCalledWith({
                method: 'eth_hashrate',
                params: []
        ...
        it('should convert RPC result to BigInt', async () => {
            ...
            const result = await web3Eth.getHashRate({ returnType: ValidTypesEnum.BigInt });
            expect(result).toStrictEqual(expectedResult);
            expect(web3ProvidersHttpRequestSpy).toHaveBeenCalledWith({
                method: 'eth_hashrate',
                params: []
