...
import { closeOpenConnection, describeIf, getSystemTestAccounts, getSystemTestBackend, isWs } from "web3/test/shared_fixtures/system_tests_utils";
import { ERC20TokenAbi, ERC20TokenBytecode } from '../fixtures/contracts/ERC20Token';

describeIf(getSystemTestBackend() === 'infura')('Black Box Unit Tests - web3.eth.Contract - Infura - ERC20', () => {
...
describeIf(getSystemTestBackend() === 'geth' || getSystemTestBackend() === 'ganache')('Black Box Unit Tests - web3.eth.Contract - Geth || Ganache - ERC20', () => {
    let accounts;
	let web3: Web3;
	...
	beforeAll(async () => {
		...
		web3 = new Web3(process.env.WEB3_SYSTEM_TEST_PROVIDER);
        deployedContract = await new web3.eth.Contract(ERC20TokenAbi).deploy({
	...
	it('should get deployed contract info', async () => {
        const contract = new web3.eth.Contract(ERC20TokenAbi, deployedContract.options.address);
