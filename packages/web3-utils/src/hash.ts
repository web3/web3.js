...
/* eslint-disable */
import { toHex, utf8ToHex } from './converters';
...
export const sha3 = (data: string): string | null => {
	...
	const hash = keccak256(Buffer.from(data)).toString('hex');
...
export const pack = (value: string): boolean => false;

export const soliditySha3 = (...values: any[]): string | null => {
	const args = Array.prototype.slice.call(values);

	const hexArgs = args.map(pack);

	return sha3(`0x${hexArgs.join('')}`);
};

export const soliditySha3Raw = (_value: string): boolean => false;
...
const parseTypeN = (type: string): number | null => {
	const typesize = /^\D+(\d+).*$/.exec(type);
	return typesize ? parseInt(typesize[1], 10) : null;
};

const parseNumber = (value: string): Numbers => {
...
export const processSolidityEncodePackedArgs = (
	...
	if (Array.isArray(val)) {
		...
		if (arraySize && val.length !== arraySize) {
			throw new Error(' is not matching the given array ');
		}
		// else {
		//     arraySize = value.length;
		// }
		const hexArg = val.map(v => solidityPack(type, v, arraySize).replace('0x', ''));
		return hexArg.join('');
	}
