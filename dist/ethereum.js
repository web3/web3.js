    ...
    var send = function (data, callback) {
        ...
        data.forEach(function(item){
            ...
            if(typeof item.inputFormatter === 'function') {
                item.params = Array.prototype.map.call(item.params, function(item, index){
                    // format everything besides the defaultblock, which is already formated
                    return (!item.addDefaultblock || index+1 < item.addDefaultblock) ? item.inputFormatter(item) : item;
                });
        ...
        if(typeof callback === 'function' && provider.name === 'RPC over HTTP'){
            provider.send(payload, function(result, status){
                ...
                result.forEach(function(item, index){
                    ...
                    // format the output
                    callback(null, (typeof data[index].outputFormatter === 'function') ? data[index].outputFormatter(item.result) : item.result);
                });
            });
