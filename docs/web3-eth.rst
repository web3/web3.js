...
``Promise`` returns ``Object|Boolean`` - A sync object when the node is currently syncing or ``false``:
    ...
    - ``startingBlock`` - ``String``: The block number where the sync started.
    - ``currentBlock`` - ``String``: The block number where at which block the node currently synced to already.
    - ``highestBlock`` - ``String``: The estimated block number to sync to.
    - ``knownStates`` - ``String``: The estimated states to download
    - ``pulledStates`` - ``String``: The already downloaded states
...
``Promise`` returns ``String`` - The number of transactions sent from the given address.
...
1. ``Object`` - The transaction object to send:
    ...
    - ``gas``  - ``Number|String|BN|BigNumber``: (optional, default: To-Be-Determined) The amount of gas to use for the transaction (unused gas is refunded).
    ...
    - ``nonce`` - ``Number|String|BN|BigNumber``: (optional) Integer of a nonce. This allows to overwrite your own pending transactions that use the same nonce.
...
``Promise`` returns ``String`` - the used gas for the simulated call/transaction.
...
- ``blockNumber`` - ``String``: The block number where this log was created in. ``null`` when still pending.
