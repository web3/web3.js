...
require("@nomicfoundation/hardhat-web3-v4"); // <==== add this line
...
This code sets up a Web3.js connection to the development network that is provided by the [Hardhat Runtime Environment (HRE)](https://hardhat.org/hardhat-runner/docs/advanced/hardhat-runtime-environment) and logs the current block number to the console.
...
Replace the contents of `index.js` with the following code to invoke the `deploy` function:
...
If everything is working correctly, you should see something like the following (note that the values will probably not be exactly the same):
